...
```
This is the working directory. The hashtag is called a comment, not a code so it's ignored. 
...
```{r}
y <- 30 #this means y gets the value 30
```
...
```{r}
mean(c(2, 8, 2)) #c stands for combine and create a vector
```
...
```{r}
new_vector <- c(7, 6.2, 5, 9, NA, 4, 9.8, 7, 3, 2) #create new vector
```
...
```{r}
mean(new_vector, na.rm = TRUE) #removes NA values (code of na.rm)
```