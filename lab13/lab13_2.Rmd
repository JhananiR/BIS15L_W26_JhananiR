...
```{r}
anti_join(table_A, table_B, by="customer_ID") #used to troubleshoot problematic joins
```
...
```{r message=FALSE, warning=FALSE}
spider_records <- read_csv("data/spiders_locations.csv") %>% clean_names()
```

```{r, message = FALSE}
spider_locs <- read_csv("data/spider_records.csv") %>% clean_names()
```
