...
```{r}
map1 <- get_stadiamap(bbox, maptype="stamen_terrain", zoom=7) #higher zoom number means more quality map
```
...
ggmap(map1)+ 
  ...
  labs(x="Longitude", y="Latitude", title="CA Usofila Record")
```
...
```{r}
ggmap(map1)+ 
  geom_point(data=grizzly, aes(longitude, latitude), size=0.8, color="black", alpha=0.8)+
  labs(x="Longitude", y="Latitude", title="Grizzly Record")
```
