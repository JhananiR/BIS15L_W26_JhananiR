...
```{r}
ggmap(map1)+
  geom_point(data=sharks, aes(x=longitude, y=latitude), size=0.75, color="black", alpha=0.8)+
  labs(x="Longitude", y="Latitude", title="CA Sharks Incidents")
```

4. Build a map that only shows the fatalities. 
```{r}
ggmap(map1)+
  geom_point(data=sharks %>% filter(injury == "fatal"), aes(x=longitude, y=latitude), size=0.75, color="black", alpha=0.8)+
  labs(x="Longitude", y="Latitude", title="CA Sharks Incidents- Fatalities Only")
```
...
ggmap(map1) +
  geom_point(data=sharks_county, aes(x=longitude, y=latitude), size=0.75, color="black", alpha=0.8) +
  labs(x="Longitude", y="Latitude", title="Shark Incidents - San Diego")
sharks_county
```
